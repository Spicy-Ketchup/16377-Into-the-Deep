...
public class Red_Teleop extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        ElapsedTime spt = new ElapsedTime();
        ...
        while (opModeIsActive()) {
            ...
            switch (slideState){
                case transfer:
                    if (gamepad1.right_bumper && pivotTime.seconds()>.25) {
                        custom = 0;
                        HarmTarget = 700;
                ...
                if (liftState == lift_State.up)
                    if (gamepad1.left_trigger>0.8 || blue){
                        servos.LeftSpin.setPower(-1);
                        servos.RightSpin.setPower(-1);
                    } else if (gamepad1.right_trigger>0.8||pivotPos){
                        servos.LeftSpin.setPower(1);
                        servos.RightSpin.setPower(1);
                    } else{
                    ...
                    if (gamepad1.options)
                        ...
                        custom = 3;
                    } else if (custom!=0 || custom == 1 || custom == 2 || custom == 3)
                        custom = 4;
            if (blue)
            ...
            lifts.update(LiftTarget, resetting);
                    slide.update(HarmTarget, custom);
